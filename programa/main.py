...
def seleccionar_empleado(empleados: list[dict]) -> dict | None:
    """Solicita el identificador del empleado hasta localizar una entrada válida."""
...
def ejecutar_sesion(
    ...
    while True:
        ...
        # 1. PREPARACIÓN (Usando el orquestador seguro)
...
def main() -> None:
    ...
    Aplica política Fail-Closed en autenticación antes de cargar datos.
    """
    # 1. Validación de Seguridad Temprana
    try:
        configurar_gemini_api_key(interactivo=True, sobrescribir=False)
    except GeminiAuthError as error:
        print(f"\n[CRÍTICO] Fallo de inicialización de seguridad:")
        print(f"- {error}")
        print("El asistente no puede operar sin credenciales válidas. Cierre seguro.")
        sys.exit(1)

    # 2. Carga limpia de datos
    try:
        datos = cargar_datos()
    except (FileNotFoundError, ValueError, OSError) as error:
        print("\nNo se ha podido iniciar la aplicación debido a un fallo en las fuentes:")
        print(f"- {error}")
        return

    # 3. Lanzamiento de la Sesión Interactiva de Producción
...
if __name__ == "__main__":
    print("1. Aplicación principal (Chat Interactivo)")
